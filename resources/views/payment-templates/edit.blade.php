...
@section('content')
	...
	<div class="card">
		<div class="card-header">
			<div class="pull-left">
                <h5>Edit Payment Template</h5>
	        </div>
        </div>

		<div class="card-body">
        <form method="post" action="{{ route('payment-templates.update', $data->id) }}" enctype="multipart/form-data">
        @csrf
		@method('PATCH')
        <div class="row">
			<div class="col-md-6">
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" name="subtitle" id="subtitle" required value="{{ $data->subtitle }}"/>
				</div>
				<div class="form-group">
					<label for="payment_type">Payment Type</label>
                    <select name="payment_type" id="payment_type" class="form-control" required>
                        <option value="0"> -- Select --</option>
						<option value="Page" {{ ('Page'==$data->payment_type)?'selected':''}}>Payment Page</option>
                        <option value="Link" {{ ('Link'==$data->payment_type)?'selected':''}}>Payment Link</option>
                        <option value="Button" {{ ('Button'==$data->payment_type)?'selected':''}}>Payment Button</option>
                    </select>
				</div>
            </div>
            <div class="col-md-6">
                ...
                <div class="form-group">
                    <img src="{{ asset('/storage/template/'.$data->bg_image)}}" style="height:80px;" />
                </div>
            </div>
            <div class="col-xs-12 col-sm-12 col-md-12">
                <div class="form-group">
					<label for="description">Page Content</label>
					<textarea name="description" id="description" rows="10" cols="80" style="visibility: hidden; display: none;">{{ $data->description}}</textarea><br/>
				</div>
            </div>
            <div class="col-xs-12 col-sm-12 col-md-12">
